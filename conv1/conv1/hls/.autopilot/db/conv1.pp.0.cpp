...
__attribute__((sdx_kernel("conv1_1d", 0))) void conv1_1d(const data_t inputs[C1_IC][C1_IN_LEN],
              data_t outputs[C1_OC][C1_OUT_LEN],
              const data_t weights[C1_OC][C1_IC][C1_K],
              const data_t bias[C1_OC]) {
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=conv1_1d
# 9 "conv1.cpp"
    ...
    VITIS_LOOP_15_1: for (int oc = 0; oc < C1_OC; ++oc) {
        VITIS_LOOP_16_2: for (int o = 0; o < C1_OUT_LEN; ++o) {
            data_t acc = bias[oc];

            VITIS_LOOP_19_3: for (int ic = 0; ic < C1_IC; ++ic) {
                VITIS_LOOP_20_4: for (int k = 0; k < C1_K; ++k) {
